...
# 加载权重
model_path = "unet.pth"  # 更改为你模型的路径
model.load_state_dict(torch.load(model_path))
model.eval()  # 设置为评估模式

# 使用 TorchScript 进行转化
example_input = torch.randn(1, 3, 512, 512)  # 根据你的模型输入调整这个形状

# 使用 torch.jit.trace 来生成 TorchScript 模型
traced_model = torch.jit.trace(model, example_input)

# 保存为 .pt 文件
output_path = "UNet_traced.pt"  # 你希望保存的路径
